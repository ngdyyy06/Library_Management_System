...
export default function BookDetailPage() {
        ...
        return (
            <div className="flex min-h-screen items-center justify-center bg-slate-50 p-6">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-sm font-semibold text-slate-500">Loading book details...</p>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="flex min-h-screen items-center justify-center bg-slate-50 p-6">
                <div className="w-full max-w-md rounded-2xl border border-slate-200/80 bg-white p-8 text-center shadow-lg">
                    ...
                    <p className="mt-1 text-xs text-slate-400">The requested book ID #{id} does not exist or has been removed.</p>
                    <button
                        type="button"
    ...
    const stockPercent = totalQty > 0 ? Math.round((availableQty / totalQty) * 100) : 0;
        ...
        <div className="min-h-screen bg-slate-50 p-6 sm:p-8 lg:p-10">
            <div className="mx-auto max-w-6xl space-y-8">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <div className="mb-2 flex items-center gap-2 text-xs font-medium text-slate-400">
                            ...
                            <span>/</span>
                            <span className="font-semibold text-slate-700 truncate max-w-xs">{book.title}</span>
                        </div>
                    ...
                    <button
                        type="button"
                        onClick={() => router.back()}
                        className="group inline-flex items-center gap-2 self-start sm:self-auto rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs sm:text-sm font-semibold text-slate-700 shadow-sm transition-all duration-150 hover:-translate-y-0.5 hover:bg-slate-50 active:translate-y-0"
                    >
                ...
                <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
                    ...
                    {/* ── Left Column: Stylized Book Presentation ── */}
                    <div className="lg:col-span-4 space-y-6">
                        ...
                        <div className="relative overflow-hidden rounded-2xl border border-indigo-100 bg-gradient-to-br from-indigo-700 via-indigo-600 to-sky-500 p-8 text-white shadow-xl shadow-indigo-200/50">
                            ...
                            <div className="relative z-10 flex flex-col items-center text-center">
                                {/* Book Icon Badge */}
                                <div className="mb-6 flex h-20 w-20 items-center justify-center rounded-2xl bg-white/15 backdrop-blur-md border border-white/20 shadow-inner">
                                ...
                                <p className="mt-2 text-xs text-indigo-100/80 font-medium">
                                    {book.publisher ? `Published by ${book.publisher}` : "Catalog Title"}
                                </p>

                                {/* Monospace ISBN */}
                                <div className="mt-6 rounded-lg bg-black/20 backdrop-blur-sm px-3 py-1.5 font-mono text-xs text-indigo-100 border border-white/15">
                                    ISBN: {book.isbn}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm">
                            ...
                            <div className="mt-4 grid grid-cols-2 gap-4">
                                <div className="rounded-xl border border-slate-100 bg-slate-50/60 p-3.5 text-center">
                                    <p className="text-[11px] font-semibold text-slate-400 uppercase">Total Copies</p>
                                    <p className="mt-1 text-2xl font-extrabold text-slate-900">{totalQty}</p>
                                </div>

                                <div className="rounded-xl border border-emerald-100 bg-emerald-50/50 p-3.5 text-center">
                                    <p className="text-[11px] font-semibold text-emerald-600 uppercase">Available</p>
                                    <p className="mt-1 text-2xl font-extrabold text-emerald-700">{availableQty}</p>
                                </div>
                            </div>
                    ...
                    <div className="lg:col-span-8 space-y-6">
                                ...
                                <div className="mt-2.5 flex flex-wrap gap-2">
                                    {book.authors && book.authors.length > 0 ? (
                                        book.authors.map((author: any, idx: number) => (
                                            <span
                                                key={idx}
                                                className="inline-flex items-center gap-1.5 rounded-xl bg-indigo-50 px-3 py-1.5 text-xs font-bold text-indigo-700 border border-indigo-100"
                                            >
                                        ...
                                        <span className="text-sm italic text-slate-400">No authors listed</span>
                                    )}
                                </div>
                            </div>
                        ...
                        <div className="rounded-2xl border border-slate-200/80 bg-white p-6 sm:p-7 shadow-sm">
                            ...
                            <p className="mt-4 text-sm leading-relaxed text-slate-600 whitespace-pre-line">
                                {book.description || "No description provided for this book title."}
                            </p>
                        </div>
