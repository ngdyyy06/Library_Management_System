...
export default function StaffReadersPage() {
    ...
    async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        if (!formData.readerCode.trim()) {
            setError("Reader code is required.");
            return;
        }
                ...
                <div className="mx-auto max-w-7xl space-y-6">
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                        ...
                        <div className="rounded-xl border border-slate-200/80 bg-white p-5 shadow-xs">
                            <p className="text-xs font-medium text-slate-500">Total Readers</p>
                            <p className="mt-2 text-2xl font-semibold tracking-tight text-slate-900">
                                {totalReaders}
                            </p>
                            <p className="mt-1 text-[11px] text-slate-400">All registered patrons</p>
                        </div>

                        {/* Active Readers */}
                        <div className="rounded-xl border border-slate-200/80 bg-white p-5 shadow-xs">
                            <p className="text-xs font-medium text-slate-500">Active Readers</p>
                            <p className="mt-2 text-2xl font-semibold tracking-tight text-emerald-600">
                                {activeReaders}
                            </p>
                            <p className="mt-1 text-[11px] text-slate-400">Eligible for borrowing</p>
                        </div>

                        {/* Inactive Readers */}
                        <div className="rounded-xl border border-slate-200/80 bg-white p-5 shadow-xs">
                            <p className="text-xs font-medium text-slate-500">Inactive Readers</p>
                            <p className="mt-2 text-2xl font-semibold tracking-tight text-slate-600">
                                {inactiveReaders}
                            </p>
                            <p className="mt-1 text-[11px] text-slate-400">Suspended or locked</p>
                        </div>
                    </div>
                    ...
                    <div className="rounded-xl border border-slate-200/80 bg-white shadow-xs">
                        ...
                        {loading ? (
                            <div className="flex flex-col items-center justify-center py-16 text-center">
                                ...
                                <p className="mt-3 text-xs text-slate-500">Loading readers directory...</p>
                            </div>
                            ...
                            <div className="flex flex-col items-center justify-center py-16 text-center">
                                ...
                                <p className="mt-3 text-sm font-medium text-slate-800">No readers found</p>
                                <p className="mt-0.5 text-xs text-slate-400">No patron records match your search or filter criteria.</p>
                            </div>
                        ) : (
                            <div className="overflow-x-auto">
                                <table className="w-full min-w-[800px] border-collapse text-left">
                                    ...
                                    {filteredReaders.map((reader) => (
                                        <tr key={reader.id} className="transition-colors hover:bg-slate-50/70">
                                                    ...
                                                    >
                                                        <span
                                                            className={`h-1.5 w-1.5 rounded-full ${
                                                                reader.status === "ACTIVE" ? "bg-emerald-500" : "bg-slate-400"
                                                            }`}
                                                        />
                                            ...
                                            <td className="px-5 py-3.5 text-right">
                                                <div className="flex items-center justify-end gap-1.5">
                                                    ...
                                                    <button
                                                        type="button"
                                                        onClick={() => router.push(`/staff/readers/${reader.id}`)}
                                                    ...
                                                    {reader.status === "ACTIVE" ? (
                                                        <button
                                                            type="button"
                                                            onClick={() => handleDeactivate(reader.id)}
                                                        ...
                                                        <button
                                                            type="button"
                                                            onClick={() => handleActivate(reader.id)}
                                ...
                                <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                                    <div>
                                        <h2 className="text-base font-semibold text-slate-900">
                                            {editingReader ? "Edit Reader Profile" : "Register New Reader"}
                                        </h2>
                                    ...
                                    <div className="grid grid-cols-1 gap-4 p-6 sm:grid-cols-2">
                                        ...
                                        <div>
                                            <label className="mb-1.5 block text-xs font-medium text-slate-700">
                                                Reader Code <span className="text-rose-500">*</span>
                                            </label>
                                            <input
                                                type="text"
                                                name="readerCode"
